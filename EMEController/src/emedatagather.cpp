    ...
    while (!stop())
    {
        ...
        catch(std::exception &e)
        {
            ATCCSExceptionHandler::addException(ATCCSException::STDEXCEPTION, "%s%d%s",
                                                gettext("Fails to resolve realtimedata. EME: "), _type, e.what());
        }
    }
}
...
unsigned long long EMEDataGather::resolveRealtimeData(std::shared_ptr<ATCCSData> data)
{
    ...
    if(!_realtimeData)
    {
        ...
        {
            ATCCSExceptionHandler::addException(ATCCSException::STDEXCEPTION, "%s%d%s",
                                                gettext("Fails to create realtimedata. EME: "), _type, e.what());
            return id;
        }
    }   
    ...
    {
        ATCCSExceptionHandler::addException(ATCCSException::STDEXCEPTION, "%s%d%s",
                                                gettext("Fails to persist realtimedata. EME: "), _type, e.what());
    }
#endif
    return id;
}
