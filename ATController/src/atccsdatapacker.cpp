...
std::shared_ptr<ATCCSData> ATCCSDataPacker::packCCDInstruction_Stop(unsigned int at)
{
    unsigned int size = sizeof (_ATCCSPHeader) + sizeof (_AT_INSTRUCTION_HEADER);
    std::shared_ptr<ATCCSData> pendingData(new (std::nothrow) ATCCSData(size));
    if (pendingData)
    {
        _ATCCSPHeader header;
        packATCCSHeader(header, size, at, CCD);
        _AT_INSTRUCTION_HEADER in;
        packInstructionHeader(in, at, CCD, 0, _CCD_INSTRUCTION_ABORTEXPOSURE);
        memcpy(pendingData->data(), &header, sizeof (_ATCCSPHeader));
        memcpy(pendingData->data() + sizeof (_ATCCSPHeader), &in, sizeof (_AT_INSTRUCTION_HEADER));
    }
    return pendingData;
...
std::shared_ptr<ATCCSData> ATCCSDataPacker::packGimbalInstruction_Stop(unsigned int at)
{
    unsigned int size = sizeof (_ATCCSPHeader) + sizeof (_AT_INSTRUCTION_HEADER);
    std::shared_ptr<ATCCSData> pendingData(new (std::nothrow) ATCCSData(size));
    if (pendingData)
    {
        _ATCCSPHeader header;
        packATCCSHeader(header, size, at, GIMBAL);
        _AT_INSTRUCTION_HEADER in;
        packInstructionHeader(in, at, GIMBAL, 0, _GIMBAL_INSTRUCTION_STOP);
        memcpy(pendingData->data(), &header, sizeof (_ATCCSPHeader));
        memcpy(pendingData->data() + sizeof (_ATCCSPHeader), &in, sizeof (_AT_INSTRUCTION_HEADER));
    }
    return pendingData;
